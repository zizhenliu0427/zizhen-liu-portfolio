...
export const zh = {
  ...
  desktop: {
    ...
    intelliDesc:
      "Pencil 原型设计、响应式 UI 开发、Agile 交付与 Scrum 内训、安全与负载测试、Python 数据爬虫。",
    goldenCompany: "重庆金夫人实业有限公司",
    goldenRole: "软件开发实习生 · 2024年3月 – 2024年7月",
    goldenDesc:
      "Next.js + React 官网前台、CMS 接口对接、服务端渲染、旧站迁移查漏、工作站支持。",
    projectsLabel: "项目",
    projectsTitle: "项目",
    sensorProject:
  ...
  projectContent: {
    ...
    mediajira: {
      ...
      highlights: [
        ...
        "多人实时协作表格:在线状态、他人光标,协作编辑约 155ms 同步",
  ...
  experienceContent: {
    codritium: {
      ...
      bullets: [
        ...
        "在动手前先否定了表面结论:审计发现 Kafka 虽在 Docker 栈里却零应用流量,而 Celery on Redis 本就到位——真正的缺陷是发送管线只有部分异步,数据库提交到消息队列的边界不可靠。因此不引入新的消息中间件,改用事务性 outbox 把这段交接做成可靠的,把 O(收件人数) 的状态写入移出 HTTP 路径,隔离聊天专用队列并接入 PgBouncer:100 人并发时 100 条消息与全部 9,900 条投递状态无一丢失,连接建立耗时从 50.88ms 降到 2.96ms。",
        "定位并修复两处静默丢消息的通道层缺陷——队列容量与过期窗口都跑在默认值上,压测中约 148,500 次投递里有 12,000 次(8%)消失,而消息均已提交、状态均记为已送达,其中一处只打一行 INFO、另一处完全没有日志;把两项参数显式设定并补上重连重放后,同一轮压测 148,500 次全部送达。",
        "在 Django Channels 上实现多人协同表格——在线状态、远端光标与单元格提交,配合 Redis 原子消费的一次性 WebSocket 票据、Lua 脚本清理在线状态,以及带抖动的指数退避重连;实测端到端传播 p95 为 155ms、光标路由 p95 为 3ms,错峰接入时 50 人并发编辑保持稳定。",
        "收紧越权访问:把归属型接口限定到请求者本人,堵住改 URL 里的 id 就能读到他人数据的漏洞;并修复会话过期后无限重试刷新令牌、却始终不登出的问题。",
        "把 12 个核心模块的数字型 URL id 迁移为可读 slug,涉及 373 个文件,每个模块走新增、回填、非空三步迁移,同时保持深链向后兼容。",
        "把 CI 从 57 分钟压到 20 分钟:并行执行让 pytest 收集用例从 3,808 条增加到 4,405 条的同时,运行时间从约 22 分钟减半到 11 分钟;并疏通卡在 10GB 配额上、实际占用 10.31GB 的 GitHub Actions 缓存——89 个条目里 79 个是合计 9.81GB 的 buildkit 层——让重复构建的缓存命中从 4 次 0 中变为每次都中;前端 lint、类型检查与 jest 也从静默放行改为阻断式门禁。",
        "接入实时 WebSocket 会话数指标,经 Prometheus 导出并配套 Grafana 看板;通过 Channels 消费者基类让全部 7 个消费者无需改动自身生命周期即可上报,并让计数在握手被拒、接受后崩溃与重复关闭等路径上都保持准确。",
        "把一次线上故障恢复过来:将 144 个迁移以空操作形式还原,随后补上部署后健康轮询,以及一道会让任何删除或重命名迁移文件的 PR 直接失败的 CI 关卡。",
      ],
    },
    intelli: {
      ...
      bullets: [
        ...
        "把业务需求转成用户故事和 API 约定,在敏捷冲刺评审中对照设计稿验证 React 组件,并编写覆盖角色、工件、冲刺事件与规模化框架的 Scrum 内训教材。",
        "对比 Pencil Project 与 Figma、Adobe XD 的功能、协作与成本后选定原型工具,产出并验证线框图,支撑前后端交界处的响应式实现。",
        "从 HTML 源码、Wireshark 抓包到 BuiltWith 指纹逐层逆向线上站点,定位出爬虫抓不到图片是因为页面使用了 CSS 背景图与 JavaScript 动态注入,而非标准的图片标签。",
        "搭建跨平台兼容性矩阵,覆盖 Windows、macOS 与 Android 上的 Edge、Chrome、Firefox 与 Safari,并在各响应式断点配合 Lighthouse 做性能与可访问性审计。",
        "用 Nikto 扫描客户站点,报告缺失的 X-Frame-Options、HSTS 与 X-Content-Type-Options 响应头以及 TLS 与证书问题,并为每一项给出可直接落地的修复指令。",
        "对同一客户站点做 100 至 10,000 并发用户的梯度压测,跟踪响应时间、错误率与吞吐量,测出 500 并发时错误率已达 83%。",
    ...
    goldenlady: {
      ...
      role: "软件开发实习生",
      bullets: [
        "基于 Next.js (Pages Router) 与 React 重做广州分公司官网前台:9 套页面模板支撑约 300 个可访问网址,覆盖 9 个导航栏目与 9 家门店。",
        "用 Axios 对接栏目、文章与预约接口,承载约 295 条 CMS 内容;并配合 Ant Design 与 SWR 完成预约表单,处理列表分页、栏目树与图片 CDN。",
        "用 getServerSideProps 做服务端渲染,让百度与微信能抓到页面标题与正文——该站没有 sitemap,服务端直出的 HTML 是搜索引擎收录的唯一入口;详情页渲染带图片与视频的 CMS 富文本,并按路由做代码拆分。",
        "对照旧织梦 (DedeCMS) PHP 站逐页查漏:缺页、路由与 301、移动端适配、表单提交失败与样式不一致。",
        "与后端对齐 CMS 字段 (tid/bid/rid),补上文档未覆盖的边角页面,站点全程保持在线。",
        "排查工作站硬件故障,为高负载的创意设计场景配置操作系统与相关软件。",
      ],
    },
  },
