...
import projectModels from './content/project-models.json';
...
const models = ["archive-cassette", "archive-assembly", ...Object.values(projectModels).map(model => `projects/${model.key}`)].map(name => {
