...
export class CardAppearance {
  ...
  prepare(group: THREE.Group) {
    for (const child of group.children) {
      ...
      const source = mesh.material as Surface;
      const mat = palette.high.clone();
      const amount = { value: 0 };
      const clarity = { value: 0 };
      mesh.material = mat;
      // Project clones own their temporary material; palette sources and shared
      // outgoing materials remain alive for their respective owners.
      if (mesh.userData.projectMaterialOwned) {
        source.dispose();
        mesh.userData.projectMaterialOwned = false;
      }
  ...
  setClarity(group: THREE.Group, value: number) {
    ...
    group.traverse((child) => {
      ...
      if (child.userData.projectPerformanceGlass) {
        // Small transmission buffers erase the project detail on phones.
        // Composite a light cover directly instead of resampling the interior.
        mat.transmission = 0;
        mat.opacity = THREE.MathUtils.lerp(.92, .1, clarity);
      }
    });
  }

  setProjectPerformanceGlass(group: THREE.Group, enabled: boolean) {
    group.traverse(child => {
      if (!(child instanceof THREE.Mesh) || child.userData.surface !== 'Frosted_Polymer') return;
      if (Boolean(child.userData.projectPerformanceGlass) === enabled) return;
      child.userData.projectPerformanceGlass = enabled;
      const mat = child.material as Surface;
