...
function choiceControl(attributes: string, label: string, value: string | number, choices: (readonly [string | number, string])[]) {
  if (isWallpaper) {
    const text = choices.find(([key]) => key === value)?.[1] ?? translateUi("自定义");
    return translateUi(`<button type="button" ${attributes} class="quality-cycle" aria-label="${label}" title="点击切换${label}" value="${value}" data-quality-choices="${escapeHtml(JSON.stringify(choices))}"><span data-quality-label>${text}</span><span aria-hidden="true">↻</span></button>`);
  }
  return `<select ${attributes} aria-label="${label}">${choices.map(([key, text]) => `<option value="${key}" ${key === value ? "selected" : ""}>${text}</option>`).join("")}${value === "custom" ? translateUi('<option value="custom" disabled selected>自定义</option>') : ""}</select>`;
}
...
export function qualityMarkup(quality: RenderQuality) {
  ...
  return translateUi(`<section class="quality-settings" aria-label="画质设置">
    ...
    ${select(
      ...
      "pixelRatio",
      translateUi("像素密度上限"),
      translateUi("控制高密度屏幕的原生像素倍率"),
      [1, 1.5, 2, 3].map((v) => [v, `${v}×`]),
    )}
    ${select(quality, "antialias", translateUi("抗锯齿"), translateUi("SMAA 平滑模型边缘与后处理结果"), [
      ["off", translateUi("原始")],
      ["smaa", "SMAA"],
    ])}
    ${select(
    ...
    ${select(
      ...
      "transmission",
      translateUi("透明材质分辨率"),
      translateUi("控制盖板折射画面的清晰度"),
      [0.25, 0.5, 0.75, 1].map((v) => [v, `${v * 100}%`]),
    )}
    ${select(
      quality,
      "shadows",
      translateUi("阴影分辨率 · 阵列"),
      translateUi("更高分辨率保留更细的投影边缘"),
      [
        [0, translateUi("关闭")],
        [1024, "1024"],
        [2048, "2048"],
    ...
    ${select(
      ...
      "aoSamples",
      translateUi("环境遮蔽 · 阵列"),
      translateUi("采样越多，接缝暗部越细腻"),
      [
        [0, translateUi("关闭")],
        [16, translateUi("16 采样")],
        [32, translateUi("32 采样")],
        [64, translateUi("64 采样")],
      ],
    )}
    ${select(
      quality,
      "aoResolution",
      translateUi("遮蔽分辨率 · 阵列"),
      translateUi("降低可减轻环境遮蔽的渲染负担"),
      [0.5, 0.75, 1].map((v) => [v, `${v * 100}%`]),
    )}
    ${range(quality, "depthOfField", translateUi("景深强度 · 阵列"), translateUi("0% 关闭；100% 保留原始镜头虚化"), 0, 150)}
    </div></details><p class="quality-note">${isWallpaper ? translateUi("即时生效，仅限当前运行；长期设置请在 Wallpaper Engine 中调整。") : translateUi("即时生效并自动保存。")}清晰度与材质设置同步至 360° 查看器。高渲染比例更适合静态观察；缓冲上限为 829 万像素，硬件限制时自动收敛。</p>
  </section>`);
}
...
export function syncQualityUI(quality: RenderQuality) {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-quality-choices]").forEach(button => {
    const choices = JSON.parse(button.dataset.qualityChoices!) as [string | number, string][];
    button.querySelector("[data-quality-label]")!.textContent = choices.find(([value]) => String(value) === button.value)?.[1] ?? translateUi("自定义");
  });
  document
