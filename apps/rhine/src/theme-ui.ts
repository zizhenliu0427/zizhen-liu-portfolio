...
import "./theme.css";
const palette = {
  ink: ["#080a08", "#e0e3dc"], muted: ["#606159", "#a6b0b1"], line: ["#aaa59a", "#536166"],
...
export function themeSettingsMarkup(preference: string) {
  return translateUi(`<div class="theme-settings"><div><strong>界面配色</strong><span>玻璃阵列随配色逐张过渡</span></div><div class="theme-choices" role="group" aria-label="界面配色"><button data-color-theme="auto" aria-pressed="${preference === 'auto'}">跟随系统</button><button data-color-theme="light" aria-pressed="${preference === 'light'}">亮色</button><button data-color-theme="dark" aria-pressed="${preference === 'dark'}">暗色</button></div></div>`);
}
