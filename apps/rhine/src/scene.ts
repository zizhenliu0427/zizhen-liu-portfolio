...
export class ArchiveScene {
  ...
  setSuperPerformance(enabled: boolean, refreshMaterials = false) {
    if (this.superPerformance === enabled && !refreshMaterials) return;
  ...
  constructor(
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      t("三维研究档案阵列，点击选择，左右拖动切列，上下拖动或滚轮切换列内档案"),
    );
  ...
  private prepareProject(meshes: THREE.Mesh[]) {
    const group = new THREE.Group();
    for (const mesh of meshes) {
      const name = mesh.userData.surface;
      if (!this.projectPalettes.has(name)) {
        this.appearance.register(name, (mesh.material as THREE.MeshPhysicalMaterial).clone());
        this.projectPalettes.add(name);
      }
      mesh.userData.projectMaterialOwned = true;
      group.add(mesh);
    }
    this.appearance.prepare(group);
    this.appearance.apply(group, ease(this.lift.value / .4));
    this.appearance.setTheme(group, this.themeAmount);
    return [...group.children] as THREE.Mesh[];
  }

  retryProjectModel() { void this.loadProject(this.projectId, true); }

  private async loadProject(id: string, retry = false) {
    if (this.projectId === id && !retry) return;
    this.projectId = id;
    const ticket = ++this.projectRequest;
    const spec = projectModel(id);
    ...
    if (!spec) return;
    try {
      const meshes = await this.projectModels.create(spec);
      if (ticket !== this.projectRequest) {
        meshes.forEach(mesh => (mesh.material as THREE.Material).dispose());
        return;
      }
      const label = this.model.children[this.model.children.length - 1];
      // The outgoing-copy path relies on the unique canvas label being last.
      this.model.remove(label);
      for (const mesh of this.prepareProject(meshes)) this.model.add(mesh);
      this.model.add(label);
      this.projectModelState = 'ready';
      this.renderState.invalidate();
    } catch (error) {
      if (ticket !== this.projectRequest) return;
      this.projectModelState = 'error';
      console.warn('Project interior could not load', spec.key, error);
    }
  }

  async createAssemblyModel() {
    const spec = projectModel(this.projectId);
    ...
    const projectInterior = spec ? await this.projectModels.create(spec) : [];
    ...
    if (spec) {
      const interior = this.prepareProject(projectInterior);
    ...
    return {
      ...
      dispose: () => {
        for (const mesh of meshes) {
          if (!mesh.userData.portfolioInterior) mesh.geometry.dispose();
    ...
    const shift = {
      ...
      row:
        Math.abs(this.selectedCell.row) > 2048
          ? Math.floor((this.selectedCell.row - 12) / archiveRowPeriod) * archiveRowPeriod
          : 0,
    };
  ...
  update(
    ...
    this.decryption.update(dt, detail > .78 && this.lift.value > 3.3 && this.projectModelState !== 'loading', this.reduced,
      cinematic ? shot + 5 : undefined, motionSpeed);
  ...
  getStats() {
    ...
    return {
      projectModel: { key: this.projectModelKey, state: this.projectModelState,
        meshes: this.model.children.filter(child => child.userData.portfolioInterior).length,
        visibleOriginalInteriors: this.model.children.filter(child => isOriginalInterior(child) && child.visible).length },
      ...
      performanceMaterials: this.instances.filter(inst => inst.material === inst.userData.fastMaterial).length,
      ...
      returningProjectModels: this.outgoing.map(old => [...new Set(old.group.children.map(child => child.userData.projectKey).filter(Boolean))]),
