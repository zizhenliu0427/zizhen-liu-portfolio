import { language, t as translateUi } from './locale';
import type { ProjectModel } from './project-models';
...
type ModelSource = { model: THREE.Group; project?: ProjectModel; dispose: () => void; setClarity?: (value: number) => void };
export class ModelViewer {
  ...
  constructor(
    ...
    this.root.innerHTML = translateUi(`
      ...
      <header class="viewer-header">
        ...
        <div class="viewer-heading"><span>ZL ARCHIVE / OBJECT STUDY</span><h2 id="viewer-title">档案模型</h2><p id="viewer-file"></p></div>
    ...
    this.renderer.domElement.setAttribute(
      "aria-label",
      translateUi("档案三维模型：拖动旋转，方向键平移，滚轮或加减键缩放，Home 复位"),
    );
  ...
  private async load() {
    ...
    loading.querySelector("span")!.textContent = translateUi("正在载入模型…");
    ...
    try {
      ...
      this.source = source;
      this.root.dataset.projectModel = source.project?.key ?? 'default';
      this.root.querySelector('.viewer-parts')!.innerHTML = `<div>${translateUi('ASSEMBLY / 装配结构')}</div>${PARTS.map((part, index) => {
        const layer = index === 2 ? 0 : index === 3 ? 1 : -1;
        const label = source.project && layer >= 0
          ? language === 'zh' ? source.project.layers[layer] : source.project.layersEn?.[layer] ?? (layer === 0 ? 'Routing & connections' : 'Project modules')
          : translateUi(part.label);
        const en = source.project && layer >= 0 ? source.project.layersEn?.[layer] ?? (layer === 0 ? 'SIGNAL LAYER' : source.project.title) : part.en;
        return `<p><span>${String(index + 1).padStart(2, '0')}</span><strong>${label}</strong><small>${en}</small></p>`;
      }).join('')}`;
      ...
      this.setExploded(false);
      this.setStatus(translateUi("已组装"));
      ...
      this.loading = false;
      loading.querySelector("span")!.textContent = translateUi("模型载入失败，请重试");
  ...
  private setExploded(value: boolean) {
    ...
    this.root
      ...
      .setAttribute("aria-pressed", String(!value));
    this.setStatus(
      value ? translateUi("正在拆解") : this.spread.value > 0.001 ? translateUi("正在重组") : translateUi("已组装"),
    );
  ...
  resize() {
    ...
    help.innerHTML = touch
      ? translateUi("<span>单指旋转</span><span>双指缩放 / 平移</span>")
      : translateUi("<span>拖动旋转</span><span>↑ ↓ ← → 平移</span><span>滚轮缩放</span>");
  }
  ...
  update(time: number) {
    ...
    if (this.source) {
      ...
      if (
        ...
        this.setStatus(this.targetSpread ? translateUi("已拆解") : translateUi("已组装"));
      }
      for (const part of PARTS) {
    ...
    this.root.dataset.stats = JSON.stringify({
      ...
      parts: [...this.groups].map(([id, group]) => ({
        ...
        evidenceScreens: group.children.filter(child => child instanceof THREE.Mesh && (child.material as THREE.MeshPhysicalMaterial).emissiveMap).length,
      })),
    });
  }
}
