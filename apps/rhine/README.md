...
- 18 个项目分别使用原创 Blender 内构，保留统一外壳、抽取与解密，并在 360° 查看器中拆解相同结构。模型按需加载，首次打开后缓存；个人资料与实习暂沿用原内构。制作与映射见 [项目模型说明](art/PROJECT-MODELS.md)。
- 保留已有档案 ID；合并后编号有空缺。旧技术专题收藏自动归入所属项目，旧 TXT 链接返回完整的所属档案。
...
- 黑客帝国版与 Win7 版保留在原独立仓库，本仓库单独构建和预览。
